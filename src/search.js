var data = [
    ...
    { nome: "João Berê", email: "joao569@gmail.com", cpf: "123.256.439-49", contato: "(11) 983905-4401" },
...
searchInput.addEventListener("input", function (event) {
    ...
    if (filteredResults.length > 0) {
        ...
        searchResultsContainer.innerHTML = filteredResults.map(function (item) { return "\n    <div class=\"px-4 py-2 border-b border-gray-200 hover:bg-gray-100 cursor-pointer\">\n      <p class=\"text-sm font-medium text-[#101828]\">".concat(item.nome, "</p>\n      <p class=\"text-sm text-[#667085]\">").concat(item.email, "</p>\n      <p class=\"text-sm text-[#667085]\">").concat(item.cpf, "</p>\n      <p class=\"text-sm text-[#667085]\">").concat(item.contato, "</p>\n    </div>\n  "); }).join('');
    }
    else {
        searchResultsContainer.classList.remove("invisible", "opacity-0");
